...
**Span payloads are never resident; indexed attribute *values* are.** Segments are file-backed and payloads are read on demand, so a store larger than RAM serves correctly — as long as what you index is low-cardinality. Measured: 10M spans with six enum-valued attributes open in 846 MiB, and the cost is just 8 bytes per span per indexed attribute. But the attribute index is keyed on the whole value, so indexed prompt text stays in RAM verbatim: one 2 KiB prompt per span measures **RSS ≈ 1.44 × the prompt text ingested**, which is O(data), not O(indexes). Lowering `--payload-threshold-bytes` keeps prompts out of the index and measured a 4.8x cut in RSS on that corpus, at the cost of a written file per distinct value. Filtered search costs one index probe per segment, which is what size-tiered compaction (on by default) exists to bound. Measured figures and their trade-offs: [capacity](docs/operations/capacity.md).
