...
fn main() {
    ...
    if let Some(stem) = emit_record {
        ...
        // The record links its raw results as a SIBLING. Linking by the full
        // stem worked only while the stem was a bare name at the repo root:
        // the link resolves relative to the markdown file, so once the record
        // moved into a directory the same string pointed at that directory
        // nested inside itself.
        let json_link = std::path::Path::new(&json_path).file_name().map_or_else(
            || json_path.clone(),
