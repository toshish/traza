...
| `--mcp-max-result-bytes N` | `32768` | Ceiling on one MCP tool result or resource read, counting the text block and `structuredContent` together. Past it, rows are dropped and the truncation is stated with the argument that would have narrowed it. Raise for a client with a large context window; lower to make an agent narrow its own queries. |
