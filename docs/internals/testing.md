...
| [`durability.rs`](../../tests/durability.rs) | 12 | The acknowledgement contract, **proven by SIGKILL** — no unwinding, no destructors, no flush on the way out. Each mode held to exactly what it claims, including that `buffered` is verified lossy rather than accidentally durable. Two kills are aimed at a specific window rather than a stopwatch: one lands inside a seal, one inside a grouped compaction merge, waiting for the journal on disk that says one is in flight. A third aims at the checkpoint's commit point from **both** sides — the manifest directory appearing, then `CURRENT` changing — because the window between them is two fsyncs wide and a stagger alone lands wherever the machine favours |
