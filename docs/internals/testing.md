...
| File | Tests | Covers |
|---|---:|---|
| [`storage.rs`](../../tests/storage.rs) | 23 | The engine end to end: persistence, crash recovery, filter correctness, ordering, TTL compaction, concurrency, second-open rejection |
...
| [`compaction.rs`](../../tests/compaction.rs) | 8 | Size-tiered compaction bounding segment count **without changing a single answer**. Every test checks the data as well as the count — losing the newest version of a re-ingested span would be far worse than the slow search it fixes |
