...
Two benchmarks, two jobs. Both build and drive the **real** release server over
its real HTTP path; neither estimates anything.
...
Neither is part of [`./ci.sh`](../../ci.sh). Run them when a change could
plausibly move performance.
