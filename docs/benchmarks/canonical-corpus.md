...
|---|---:|---:|---|
| Sustained batched HTTP ingest (durability=wal, compaction fanout=4, max segment bytes=268435456) | 65749 spans/s | >= 50,000 spans/s | PASS |
...
- Build: Cargo release profile. Timestamp: Unix 1788687642.
- Machine context: macos/aarch64, 10 available hardware threads.
- Load conditions: 1-minute load average unavailable at the end of the run — ambient desktop load, not an idle host (the house rule is [ingest.md's](ingest.md#load-conditions)). An idle rerun will likely improve the tails; the gate tripwires, not these point estimates, are the contract.
- Final server stats: `{"buffer_age_seconds":null,"buffered_records":0,"bytes_on_disk":66854329,"durability":"wal","persisted_records":1000000,"record_count":1000000,"segment_count":65,"total_records":1000000,"wal_bytes":0}`.
