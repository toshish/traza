...
One exception to "almost no disk", inherited from every format migration
([segment-format.md](../segment-format.md#migration-v6-and-v7--v8) — the
v6 → v7 upgrade did this, and the v7 → v8 upgrade does it again): a pin that
pre-dates a migration is rewritten by it, file by file, into a **full
...
the migration (on a current-format store) are cheap again. When the disk
cannot afford one full copy per pre-existing pin, release the pins before
upgrading and re-take them after.
