...
| Service | Shape |
|---|---|
| `support-agent` | Multi-turn chats, a stable `session.id` per conversation. Each turn: an `agent.turn` root, a `kb.search` tool call, and a model call (`gpt-4o-mini` / `claude-sonnet-4-5` / `gemini-2.0-flash`, some streaming) carrying `gen_ai.input.messages` / `gen_ai.output.messages`, so the conversation view renders a readable chat |
| `research-swarm` | Every 15-20 s, one fan-out: a planner model call, then 4-8 **genuinely concurrent** workers doing `web.fetch` (with the occasional 503 and retry), then a reduce call. The waterfall showpiece — its `#/trace/…` link prints when it lands |
| `code-agent` | Tool-heavy, longer traces: `fs.read`, a model call, `tests.run` (which sometimes fails), `git.diff` |
...
analytics shows crisp metered costs, not derived approximations.

## The beats

1. **Banner** — deep links to `#/tail`, `#/overview`, `#/analytics`. Open
   them before the stream starts; the 15-minute overview window fills in
   front of you because the timestamps are now, not replayed history.
...
4. **Verification at ~6 s**, every number read back from the server, exit
   non-zero if any fails: `/v1/sessions` lists at least 3 sessions,
   `/v1/stats/llm` has rows with `cost_usd > 0`, the first research trace has
   at least 6 spans with measured worker concurrency, and one session's chat
   turns parse back in the exact shape the conversation view renders —
   `{role, parts: [{type, content}]}`, every part checked for both keys.
5. **Summary on exit** (Ctrl-C or the duration knob), split into what it
   is: server-read figures (spans acked, sessions stored from
   `/v1/sessions`, total cost and top model by spend from `/v1/stats/llm`)
   and the generator's own tallies (sessions started, fan-outs, error
   spans emitted, spans still in flight).
...
| `TRAZA_SWARM_CHATS` | `4` | Concurrent support conversations (minimum 3, so the session assertion stays meaningful) |
...
- **Two kinds of numbers print, labeled as what they are.** The server-read
  ...
  the fan-out notice's worker and span counts, error spans emitted, spans
  still in flight — are its emission counters: honest bookkeeping of what it
  sent, not server measurements.
- Roughly 2% of spans are injected errors (failed fetches, failed test runs,
  the odd tool timeout); the exact emitted count is printed in the summary.
...
at ~6 s, then it streams until Ctrl-C (the summary prints on the way out).
Smoke mode (`TRAZA_SWARM_SECONDS=8`) finishes, verification and summary
included, in about 9 seconds.
